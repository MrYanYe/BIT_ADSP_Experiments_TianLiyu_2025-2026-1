...
% 基本参数
N = 500;                       % 时间轴长度
M = 20;                        % 固定脉冲宽度（将 M 固定为单值）
Noise_Amplitude_array = [ 0.1, 0.3, 1,5,10];   % 噪声标准差数组
N0 = 248;                      % 实际目标延时（回波起始索引）
Signal_Amplitude = 1;          % 脉冲幅度

numNoise = numel(Noise_Amplitude_array);
SNR_array = Signal_Amplitude ./ Noise_Amplitude_array;

% 创建画布：每个噪声一行，3 列
figure('Color','w','Units','normalized');
t = tiledlayout(numNoise,3,'TileSpacing','compact','Padding','compact');

for in = 1:numNoise
    Noise_Amplitude = Noise_Amplitude_array(in);
    ...
    title(sprintf('NoiseStd = %.3f  回波 (M = %d)', Noise_Amplitude, M));
    xlabel('样点索引');
    ylabel('幅度');
    xlim([1 N]);
    ...
    title(sprintf('NoiseStd = %.3f  接收信号 (SNR=%.2f)', Noise_Amplitude, SNR_array(in)));
    xlabel('样点索引');
    ylabel('幅度');
    xlim([1 N]);
    grid on;
    ...
    title(sprintf('NoiseStd = %.3f  相关输出', Noise_Amplitude));
    xlabel('样点索引');
    ylabel('相关幅度');
    xlim([1 N]);
    % 自适应 ylim，避免为零或 NaN
...
% 总标题
sgtitle(sprintf('固定 M=%d，多个噪声水平的匹配滤波结果（SignalAmp=%.2f，N=%d）', ...
    M, Signal_Amplitude, N));
