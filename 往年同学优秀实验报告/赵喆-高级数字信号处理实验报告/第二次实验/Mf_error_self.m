...
vari_temp = var(delayestimate_error);
vari_value = [vari_value vari_temp];
end

plot(s_range,vari_value);
figure;
subplot(221)
plot(echo_signal)
title('echo signal')
...
title('error of delay estimation');
variance = var(delayestimate_error);
mean_value = mean(delayestimate_error);
